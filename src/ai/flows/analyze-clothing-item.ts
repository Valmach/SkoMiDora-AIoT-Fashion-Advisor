 ...
 *
 * - analyzeClothingItem - A function that analyzes a clothing item image.
...
const promptObject = ai.definePrompt({
  ...
  Focus ONLY on what is visually apparent in the image. Do not infer brand unless a logo or highly distinctive, unambiguous brand marker is clearly visible.

  Image to analyze: {{media url=imageUri}}

  Based on the image, provide the following:
  - **itemName**: A concise, descriptive name for the item (e.g., "Blue Distressed Denim Jacket", "Black Quilted Leather Shoulder Bag", "Beige Suede Ankle Boots"). Aim for 3-5 words. Include brand if it's part of common parlance for the item AND clearly identifiable.
  - **itemType**: Categorize accurately from the enum: "Top", "Bottom", "Outerwear", "Dress", "Shoes", "Accessory", "Bag", "Other".
  - **designerName**: Identify the designer or brand ONLY if a clear logo, brand name, or extremely distinctive brand-specific hardware/design element is visible in the image. If not clearly identifiable, OMIT this field. Do NOT guess.
  - **color**: Describe the dominant color(s). Be specific (e.g., "Navy Blue with White Pinstripes", "Cream", "Forest Green", "Multicolour Abstract Print", "Metallic Silver").
  - **generalMaterial**: State the primary material as it appears visually (e.g., "Denim", "Silk-like Fabric", "Smooth Leather", "Chunky Knit", "Patent Finish", "Canvas").
  - **styleKeywords**: Provide 3 to 5 relevant style keywords that describe its aesthetic (e.g., "vintage-inspired", "minimalist", "bohemian-chic", "streetwear-utility", "classic-formal", "athleisure", "punk-rock", "preppy").
  - **narrativeDescription**: Write 2-3 engaging sentences describing the item's key visual features, silhouette, and overall style impression.
  - **detailedSpecifications**: List other notable visual details. This can include pattern (e.g., "Pattern: Floral Print"), print, fit (e.g., "Fit: Oversized"), embellishments (e.g., "Embellishments: Pearl Buttons"), hardware details (e.g., "Hardware: Gold-tone chain strap"), or specific design features (e.g., "Features: Notched lapels, Double-breasted"). If the country of origin is visible on a label in the image (e.g., "Made in Italy"), include it as a detail: "Origin: Made in Italy". If multiple distinct details, present them as a single string with points separated by newlines (e.g., "Pattern: Checkered\\nFit: Regular\\nDetails: Patch Pockets"). Omit if no other significant details are apparent.

  CRITICAL: Adhere strictly to the output JSON schema. Only include fields that you can confidently determine from the visual information in the image.
  If a field like 'designerName' cannot be determined, it should be omitted from the output JSON (as per its 'optional' schema definition).
  Ensure 'styleKeywords' is an array of strings.
  `,
});
