...
// 1. We move the main logic into an inner component so Next.js can safely read the URL
function ClosetContent() {
  ...
  // 2. dynamically grab the event and weather from the URL (with fallbacks)
  const eventName = searchParams.get('event') || "Select an event from your Calendar"; 
  const weatherContext = searchParams.get('weather') || "Waiting for weather data..."; 

  const handleGenerateLooks = async () => {
    if (eventName === "Select an event from your Calendar") {
      toast({ title: "No Event Selected", description: "Please initiate styling from the Events page.", variant: "destructive" });
      return;
    }
  ...
  if (!firebase) return <div className="flex justify-center items-center h-[85vh]"><Loader2 className="h-8 w-8 animate-spin text-muted-foreground" /></div>;
    ...
    <div className="container mx-auto space-y-8 pb-12 h-[85vh] overflow-y-auto">
      ...
      <div className="w-full bg-card p-6 rounded-2xl border shadow-sm">
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-4">
          ...
          <Button onClick={handleGenerateLooks} disabled={isStyling || eventName === "Select an event from your Calendar"} className="bg-black text-white hover:bg-gray-800">
            {isStyling ? <><Loader2 className="mr-2 h-4 w-4 animate-spin" /> Styling...</> : "Find Missing Pieces"}
          </Button>
        </div>
        {recs.length > 0 && <ShoppingRecommendations eventContext={eventName} recommendations={recs} />}
      </div>
      ...
      {error && <div className="flex items-center gap-2 text-destructive"><AlertCircle className="h-5 w-5" />{error}</div>}
            ...
            return (
              <div key={item.id} className="bg-card p-5 rounded-2xl border shadow-sm space-y-4">
                ...
                <div className="rounded-xl bg-muted flex items-center justify-center min-h-[300px] overflow-hidden">
                  {!url || isBroken ? (
                    <div className="flex flex-col items-center justify-center"><ImageOff className="h-10 w-10 opacity-30" /><span className="text-xs text-muted-foreground">Image unavailable</span></div>
                  ) : (
                    <Image src={url} alt={item.itemName ?? "Closet item"} width={360} height={360} className="object-contain max-h-[300px] w-auto transition-transform duration-300 hover:scale-105" unoptimized />
                  )}
                </div>
                <Button variant="destructive" onClick={() => handleDelete(item)} className="w-full"><Trash2 className="h-4 w-4 mr-2" />Remove</Button>
                <div className="space-y-3 pt-4">
                  <div className="flex items-center space-x-3"><Tag className="h-5 w-5 text-accent flex-shrink-0" /><span className="font-semibold text-sm">Type:</span><Badge variant="secondary" className="capitalize">{item.itemType || "Uncategorized"}</Badge></div>
                  <div className="flex items-center space-x-3"><Palette className="h-5 w-5 text-accent flex-shrink-0" /><span className="font-semibold text-sm">Colour:</span><span className="text-sm text-muted-foreground">{item.color || "Not specified"}</span></div>
                </div>
              </div>
            );
          })}
...
// 3. The exported page is wrapped in Suspense to satisfy Next.js build requirements
export default function ClosetPage() {
  return (
    <Suspense fallback={<div className="flex justify-center items-center h-[85vh]"><Loader2 className="h-8 w-8 animate-spin" /></div>}>
      <ClosetContent />
    </Suspense>
  );
}