...
async function analyzeImageMetadata(
  ...
  try {
    ...
    const promptExample = {
      ...
      category: `one broad top-level category from: ${[
        ...WARDROBE_CATEGORIES,
        "Uncategorized",
      ].join(" | ")}`,
      ...
      formality:
        "casual | smart-casual | business-casual | cocktail | formal",
    ...
    const prompt = [
      ...
      "itemType must be exactly one specific value from the supplied canonical item-type taxonomy. category must be the matching broad top-level wardrobe category. For example, itemType Trainers maps to category Footwear, itemType Pajama Set maps to category Sleepwear & Loungewear, and itemType Jumpsuit maps to category Jumpsuits & Rompers. Use Uncategorized only when the item cannot be classified reliably.",
...
function inferLensMetadata(fileName: string) {
  ...
  const color =
    foundColor
      ? foundColor.charAt(0).toUpperCase() +
        foundColor.slice(1)
      : "Unknown";
  ...
  try {
    ...
    const commercialMetadata = {
      ...
      ...(
        ...
        typeof body.commercialMetadata ===
          "object" &&
        !Array.isArray(
          body.commercialMetadata
        )
      ...
      ...(
        ...
        typeof body.awesomeScreenshotMetadata ===
          "object" &&
        !Array.isArray(
          body.awesomeScreenshotMetadata
        )
    ...
    // Persist a broad top-level category separately from the specific item type.
    //
    // Example:
    //   category: "Footwear"
    //   itemType: "Trainers"
    //
    // The final category is derived server-side from the canonical item type so
    // Gemini or external commercial metadata cannot reintroduce category=itemType.
    ...
    const docRef =
      await db
        ...
        .add({
          ...
          // Specific subtype:
          // Trainers, Pumps, Pajama Set, Jumpsuit, etc.
          itemType:
            canonicalItemType,
          ...
          // Broad shared category:
          // Footwear, Tops, Sleepwear & Loungewear, etc.
          category:
            canonicalCategory,
          ...
          detailedSpecifications:
            metadata.detailedSpecifications,

          narrativeDescription:
            metadata.narrativeDescription,
